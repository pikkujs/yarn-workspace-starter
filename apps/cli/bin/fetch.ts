...
export const fetch = async (serverUrl: string, apiKey?: string) => {
  ...
  if (apiKey) {
    ...
    const session = await vrameworkFetch.api('/login', 'post', {
      name: 'Yasser',
    })
  ...
  const todoViaParam = await vrameworkFetch.api('/todo/:todoId', 'get', {
    todoId: todos[0].todoId,
  })
  console.log(`Param todo: ${todoViaParam.text}`)

  const todoQuery = await vrameworkFetch.api('/todo', 'get', {
    todoId: todos[0].todoId,
  })
